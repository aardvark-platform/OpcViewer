...
let main argv = 
    ...
    //let version = CrackDetectionDll.GetDllVersion()
    //assert (version = 1u)

    //let ret = CrackDetectionDll.Init(@".\crackDetection", @".\crackDetection")
    //assert (ret = 0)
   ...
   // CooTransformation.initCooTrafo ()
    ...
    let argsList = 
        List.fold(fun (x:string) (y : string)-> x + " " + y) String.Empty (argv |> Array.toList)
    ...
    let instance = 
        OpcSelectionViewer.App.app opcDir axisFile rotate |> App.start
    //let instance = OpcOutlineTest.OutlineApp.appOutlines opcDir |> App.start 

    CrackDetectionApp.initialize()
