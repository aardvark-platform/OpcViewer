...
module QueryApp =
    ...
    let filterBySol (minsol:float) (maxsol:float) (features:plist<Feature>) =
        let range = new Range1i(minsol |> int, maxsol |> int)
    ...
    let viewQueryInstruments (grouped : IMod<list<Instrument * list<Feature>>>) (model :MQueryModel) =
        let counts = 
            grouped 
                |> Mod.map(fun groups -> 
                    groups 
                    |> List.map(fun (instrument, features) -> 
                        instrument, features.Length
                    ) |> HMap.ofList
                ) |> AMap.ofMod
    ...
    let viewQueryFilters (grouped : IMod<list<Instrument * list<Feature>>>) (model :MQueryModel) =
      require Html.semui ( 
        Html.table [                 
            ...
            Html.row "instruments:" [ (viewQueryInstruments grouped model)]
            Html.row "distance:" [Numeric.view' [NumericInputType.Slider; NumericInputType.InputBox] model.distance |> UI.map (fun x -> SetDistance x)]
