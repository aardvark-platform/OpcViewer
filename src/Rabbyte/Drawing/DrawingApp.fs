...
module DrawingApp =
    ...
    let rec update (model: DrawingModel) (act: DrawingAction) =
        ...
        | ChangeColorPrimary c1 -> 
            { model with style = { model.style with primary = c1 }}
        | ChangeColorSecondary c2 -> 
            { model with style = { model.style with secondary = c2 }}
        | ChangeColorAuto primary -> 
            let secondary = (SgUtilities.createSecondaryColor primary)
    ...
    let drawContourWithPointSize (points: alist<V3d>) (segments: alist<Segment>) (style: AdaptiveBrushStyle) (near: aval<float>) (far: aval<float>) (pointSize: aval<float>) (depthOffset : aval<float>)=  
        ...
        let pointsSg = 
            points 
            |> SgUtilities.drawPointList style.primary pointSize depthOffset near far

        let pointsInnerSg = 
            segments
            |> AList.map (fun x -> x.innerPoints |> AList.ofIndexList) 
            |> AList.collect id 
            |> SgUtilities.drawPointList (style.primary |> AVal.map (fun c -> SgUtilities.createSecondaryColor c)) (pointSize |> AVal.map (fun x -> x * 0.8)) depthOffset near far

        let edgesSg = 
            let lineWidth = style.thickness |> AVal.map (fun x -> x * 1.1)
            let sPoints = allSegmentPoints segments
            sPoints
            |> SgUtilities.lines' (depthOffset|> AVal.map (fun x -> x / 1.9)) style.secondary lineWidth near far

        let edgesDirectSg = 
            points 
            |> SgUtilities.lines' (depthOffset|> AVal.map (fun x -> x / 2.0)) style.primary style.thickness near far
    ...
    let viewGui (model: AdaptiveDrawingModel) = 
        ...
        table [clazz "item"] [
            ...
            tr [] [
                ...
                td [style style'] [Dropdown.dropdown ChangeAreaStyle false None model.style.areaStyle [ clazz "ui inverted selection dropdown" ] (model.areaStyleNames |> AMap.map (fun k v -> text v))]
            ]
            tr [] [
                td [style style'] [text "LineStyle:"]
                td [style style'] [Dropdown.dropdown ChangeLineStyle false None model.style.lineStyle [ clazz "ui inverted selection dropdown" ] (model.lineStyleNames |> AMap.map (fun k v -> text v)) ]
            ]
            tr [] [
                td [style style'] [text "PrimaryColor:"]
                td [style style'] [ColorPicker.view ColorPicker.Config.Default ChangeColorPrimary model.style.primary]
            ]
            tr [] [
                td [style style'] [text "SecondaryColor:"]
                td [style style'] [ColorPicker.view ColorPicker.Config.Default ChangeColorSecondary model.style.secondary]
            ]
        ]